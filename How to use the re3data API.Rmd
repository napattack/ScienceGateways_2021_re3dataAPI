...
The XPath expressions defined here will extract the re3data IDs, names, URLs, API endpoints and AOI types in their specific order (using the parameter *APICount*). Results are stored in a named list that can be processed later.
...
Now that the results are processed, they can be visualized. The example below firstly group data by API Type and count the frequency of repository within each group, and order the API type by occurrence of API type  descendingly. And then generate a bar chart showing the frequency of API Type in re3data, from the most frequent to the least frequent. 
